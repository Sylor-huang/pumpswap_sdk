...
const getPoolsWithPrices = async (connection: Connection, mintAddress: PublicKey) => {
    ...
    const results = await Promise.all(pools.map((k:any) => getPriceAndLiquidity(connection, k)));
...
export const getPumpSwapPool = async (connection: Connection, mint:PublicKey) => {
